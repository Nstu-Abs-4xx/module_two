...
/* int main(void)
 {
     ...
     int a = 56784, n, s, k;
     for (n = a, s = 0; n != 0; n = n / 10)//Цикл переберебирает все элементы массива
     {
         k = n % 10;
         if (k > s)
         {
         ...
         printf("%d", s);*///Программа прибавляет сумму цифр числа к его исходному значению
//this is 14
/* int main(void)
 {
     int a = 1230, n, s, k;
     for (n = a, s = a; n != 0; n = n/10)//Цикл переберебирает все элементы массива
     {
         k = n % 10;
         s = s + k;
     }
     printf("π%d and %d\n", s, k);
 }*/// Программа находит наибольшую цифру из заданного числа
//this is 15
/*int main(void)
 {
     int a = 987654321, s, k, n;
     ...
     printf("%d", s);
 }*/// Программа записывает цыфры числа в обратном порядке
//this is 16
//
/*
int main(void)
{
    int j, s, a, n, k, A[10]; // Инициализация переменных:
                              // j - индекс для массива A[],
                              // s - сумма цифр числа,
                              // a - текущее проверяемое число,
                              // n - копия числа a для манипуляций,
                              // k - текущая цифра числа n,
                              // A[10] - массив для хранения результатов.

    // Инициализация цикла, где переменные j и a принимают начальные значения 0 и 10 соответственно.
    // Цикл будет работать, пока значение a меньше 30000.
    for (j = 0, a = 10; a < 30000; a++)
    {
        // Копируем значение a в n и обнуляем s для накопления суммы цифр.
        for (n = a, s = 0; n != 0; n = n / 10)
        {
            k = n % 10;  // Получаем последнюю цифру числа n.
            s = s + k;   // Прибавляем цифру k к сумме s.
        }

        // Если текущее значение a равно кубу суммы цифр, то сохраняем его в массив A.
        if (a == s * s * s)
        {
            A[j++] = a;  // Сохраняем значение a в массиве A и увеличиваем индекс j.
            printf("Число %d соответствует условию (куб суммы цифр: %d)\n", A[j-1], s);
            // Печать значения, удовлетворяющего условию.
        }
    }
    return 0;
}
//Задача 17 берет результат куба числа,сравнивает сумму цифр этого числа с исходным числов и если равенство верное выписавыет результат куба

int main(void) {
    setlocale(LC_ALL, "Russian"); // Устанавливаем локаль для отображения русского текста
    int A[100], n, i, j; // Объявляем массив A и переменные n (размер массива), i и j (индексы)

    printf("Введите размер массива: ");
    scanf_s("%d", &n); // Считываем размер массива

    printf("Введите элементы массива:\n");
    for (i = 0; i < n; i++)
    {
        scanf_s("%d", &A[i]); // Заполняем массив элементами, введёнными пользователем
    }

    // Удаление отрицательных элементов массива
    for (i = 0; i < n; i++)
    {
        if (A[i] < 0) // Проверяем, является ли текущий элемент массива отрицательным
        {
            // Если элемент отрицательный, сдвигаем все последующие элементы влево
            for (j = i; j < n - 1; j++) {
                A[j] = A[j + 1]; // Перемещаем следующий элемент на позицию текущего
            }
            n--; // Уменьшаем размер массива, так как один элемент был удалён
            i--; // Корректируем индекс i, чтобы повторно проверить текущий индекс
        }
    }

    printf("Итоговый массив:\n");
    for (i = 0; i < n; i++) {
        printf("%d ", A[i]); // Выводим элементы итогового массива
    }

    return 0;
}

 */ //Программа берет результат куба числа,сравнивает сумму цифр этого числа с исходным числов и если равенство верное выписавыет результат куба
// this is 18
/* int main(void) {
    int v = 100;  // предел числа, до которого будем искать палиндромы
    int A[100];   // массив для хранения найденных палиндромов
    int j = 0, a, n, s, k;  // переменные для работы в цикле

    // Основной цикл перебирает все числа от 10 до v-1
    for (a = 10; a < v; a++) {
        // Проверка, является ли число палиндромом
        for (n = a, s = 0; n != 0; n = n / 10) {
            k = n % 10;     // выделяем последнюю цифру числа
            s = s * 10 + k; // строим число в обратном порядке
        }

        // Если число совпадает с перевернутым числом, то это палиндром
        if (a == s) {
            A[j++] = a; // сохраняем палиндром в массив, увеличиваем индекс
        }
    }

    // Вывод всех найденных палиндромов от 10 до v
    printf("Палиндромы от 10 до %d:\n", v);
    for (int i = 0; i < j; i++) {
        printf("%d ", A[i]); // печать каждого палиндрома из массива A
    }
    printf("\n");

    return 0;
}
 *///Программа находит числа палиндромы в пределах от 10 до заданного v 
//this is 19
/*
int main(void) {
    int A[100], n, i, j;  // Объявляем массив A на 100 элементов и переменные n, i, j
    ...
    // Цикл для удаления отрицательных элементов из массива
    for (i = 0; i < n; i++) {
        if (A[i] < 0) {  // Проверка, является ли элемент отрицательным
            // Сдвигаем все элементы массива влево, начиная с индекса i
            for (j = i; j < n - 1; j++) {
                A[j] = A[j + 1];
            }
            n--;  // Уменьшаем размер массива на 1
            i--;  // Корректируем индекс i для проверки нового элемента на этой позиции
        }
    }

    // Вывод результирующего массива без отрицательных элементов
 ...
 */ //Программа создает массив и заполняет его элементами
//this is 20
/*
 int main(void) {
    int A[] = {3, 7, 2, 9, 5};
...
*/ //Программа удаляет максимальный элемент массива
//this is 21
/*
 int main(void) {
